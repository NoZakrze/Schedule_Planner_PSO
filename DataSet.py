...
class DataSet:
    def __init__(self):
        ...
        self.rozkladZajec = [5, 4, 3, 3, 2, 2, 2, 2, 1, 1, 0,
                             0]  # rozkład ile lekcji ma jakaś klasa z danym nauczycielem
        ...
        # używamy tutaj wektorów więc zeby sprawdzić jaką lekcje ma klasa 1 we wtorek na 3 lekcji trzeba
    ...
    def GetZajecia(self, czyj_plan, indeks, dzien_tygodnia,
                   lekcja):  # zwraca komórkę z danej tablicy, wszystkie parametry indeksowane od zera
        if czyj_plan == KLASA:
        ...
        if dzien_tygodnia != None:
            ...
            return tablica[(
                               indeks) * self.tydzien + lekcja]  # jeżeli nie podamy dnia tygodnia to można bezpośrednio id zajęcia względem początku tygodnia (np. lekcja == 9 zwróci nam drugą lekcję wtorku)

    def GetDzien(self, czyj_plan, indeks,
                 dzien_tygodnia):  # zwraca listę komorek z calego dnia z danej tablicy , wszystkie parametry indeksowane od zera
        if czyj_plan == KLASA:
            tablica = self.TablicaKlas
        elif czyj_plan == NAUCZYCIEL:
            tablica = self.TablicaNauczycieli
            ...
            tablica = self.TablicaSali
        return [tablica[(indeks) * self.tydzien + self.zajecia * (dzien_tygodnia) + lekcja] for lekcja in
                range(self.zajecia)]
    ...
    def GetTydzien(self, czyj_plan,
        ...
        if czyj_plan == KLASA:
            ...
            tablica = self.TablicaSali
        return [tablica[(indeks) * self.tydzien + self.zajecia * (dzien) + lekcja] for lekcja in range(self.zajecia) for
                dzien in range(DNI_TYGODNIA)]
            ...
            for j in range(len(self.rozkladZajec)):  # pętla dla każdego nauczyciela
                ...
                for _ in range(self.rozkladZajec[nauczyciel]):  # pętla która dla nauczyliela i klasy ustala
                    ...
                    while (not timeAccept):  # tutaj w ifie sprawdzam czy klasa i nauczyciel mają tutaj wolne
                        ...
                        if (self.TablicaKlas[time + offset] == OKIENKO
                                and self.TablicaNauczycieli[j * self.tydzien + time] == OKIENKO):
                            timeAccept = True
                    ...
                    self.TablicaKlas[time + offset] = (
                    j, room)  # w tych liniach i to indeks klasy, j nauczyciela, room sali
    ...
    def Premia_za_ilosc_godzin(self, dzien, min,
                               max):  # zwraca true jeżeli ilość godzin w dniu należy do przedziału [min,max] włącznie
        count = 0
        for lekcja in range(len(dzien)):
            if [lekcja][0] != -1:
                count += 1
    ...
    def Simple_fitness(
            self):  # patrzymy tylko na to ile bloków zajęć mają łącznie klasy i w jakich godzinach mają zajęcia
        godziny_values = [i * 2 for i in range(self.zajecia // 2)] + [(self.zajecia - i) for i in
                                                                      range(self.zajecia // 2)]
        # ^sample [0, 2, 4, 6, 8, 7, 6, 5] im więcej tym lepsza godzina
        bloki = 0
        przyjazne_godziny_score = 0
    ...
    def Better_fitness(self):
        godziny_values = [i*2 for i in range (self.zajecia//2)] + [(self.zajecia - i) for i in range (self.zajecia//2)]
        bloki = 0
        ...
        #print(premia_za_rozkład_godzinny*5, przyjazne_godziny_score//5,  dni_wolne*100, bloki_przedmiotowe*50, bloki*2)
        fitness = premia_za_rozkład_godzinny*5 + przyjazne_godziny_score*5 + dni_wolne*10 + bloki_przedmiotowe*50 - bloki*30
        #print(fitness)
        return fitness
    ...
    def Get_free_rooms(self, dzien, lekcja):  # zwraca listę sali które są wolne w podanym terminie
        wolne_sale = []
        for i in range(self.sale - 1):
            if self.GetZajecia(SALA, i, dzien, lekcja) == OKIENKO:
                wolne_sale.append(i)
        return wolne_sale

    def Get_wspolne_okienka(self, nauczyciel,
                            klasa):  # zwraca listę wolnych terminów dla klasy i nauczycieli NIE PATRZAC NA SALE
        wspolne_okienka = []
